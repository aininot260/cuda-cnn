...
void init_params_gpu()
{
    ...
    init_conv_b<<<block1,grid1,0,stream[0%N_STREAM]>>>(rand());
    init_conv_w<<<block2,grid2,0,stream[1%N_STREAM]>>>(rand());
    init_fc1_b<<<block3,grid3,0,stream[2%N_STREAM]>>>(rand());
    
    for(int i=0;i<FC1_SIZE;i++)
        init_fc1_w<<<block4,grid4,0,stream[(i+3)%N_STREAM]>>>(rand(),i);
    init_fc2_b<<<block5,grid5,0,stream[8%N_STREAM]>>>(rand());
    init_fc2_w<<<block6,grid6,0,stream[9%N_STREAM]>>>(rand());
}