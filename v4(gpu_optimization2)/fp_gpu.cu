...
__global__ void _set_input_train(int idx)
{
    ...
    if(ix<ROW&&iy<COL)
    {
        _input[idx%N_STREAM][ix][iy]=_train_image[idx][ix][iy];
    }
}
...
__global__ void _set_input_test(int idx)
{
    ...
    if(ix<ROW&&iy<COL)
    {
        _input[idx%N_STREAM][ix][iy]=_test_image[idx][ix][iy];
    }
}
...
__global__ void _input_conv(int idx)
{
    ...
    if(ix<CONV_W_NUM&&iy<CONV_SIZE&&iz<CONV_SIZE)
    {
        ...
        for(int m=0;m<CONV_W_SIZE;m++)
            tile[iy][iz]+=__ldg(&_input[idx%N_STREAM][iy+l][iz+m])*_conv_w[ix][l][m];
        tile[iy][iz]+=_conv_b[ix];
        _conv_z[idx%N_STREAM][ix][iy][iz]=tile[iy][iz];
        _conv_a[idx%N_STREAM][ix][iy][iz]=_sigmoid(tile[iy][iz]);
    }
}
...
__global__ void _pool_fc1(int idx)
{
    ...
    if(i<FC1_SIZE)
    {
        ...
        for(int l=0;l<POOL_SIZE;l++)
            tile[i]+=_pool[idx%N_STREAM][j][k][l]*_fc1_w[i][j][k][l];
        tile[i]+=_fc1_b[i];
        _fc1_z[idx%N_STREAM][i]=tile[i];
        _fc1_a[idx%N_STREAM][i]=_sigmoid(tile[i]);
    }
}
...
__global__ void _fc1_fc2(int idx)
{
    ...
    if(i<FC2_SIZE)
    {
        ...
        for(int j=0;j<FC1_SIZE;j++)
            tile[i]+=_fc1_a[idx%N_STREAM][j]*_fc2_w[i][j];
        tile[i]+=_fc2_b[i];
        _fc2_z[idx%N_STREAM][i]=tile[i];
        _fc2_a[idx%N_STREAM][i]=_sigmoid(tile[i]);
    }
}
...
__global__ void _set_answer_train(int idx)
{
    ...
    if(i<FC2_SIZE)
    {
        ...
        _answer[idx%N_STREAM][i]=(_train_label[idx]==i)?1:0;
    }
}
...
__global__ void _set_answer_test(int idx)
{
    ...
    {
        _output[idx%N_STREAM][i]=_fc2_a[idx%N_STREAM][i];
        _answer[idx%N_STREAM][i]=(_test_label[idx]==i)?1:0;
    }
}
...
__global__ void _check_answer_get_error(int idx)
{
    ...
    if(_answer[idx%N_STREAM][max_pos])
        atomicExch(&_correct_cnt,_correct_cnt+1);
    for(int i=0;i<FC2_SIZE;i++)
    {
        _C[idx%N_STREAM][i]=_output[idx%N_STREAM][i]-_answer[idx%N_STREAM][i];
        atomicExch(&_avg_error,_avg_error+_C[idx%N_STREAM][i]*_C[idx%N_STREAM][i]*0.5);
    }
}

void check_answer_get_error_gpu(int idx)
{
    _check_answer_get_error<<<1,32,0,stream[idx%N_STREAM]>>>(idx);
}